@echo off
REM Build layout-probe with MSVC (x86 - matches the Win32 server/client build).
REM Emits layout.json for the spike client. Does not modify SOURCE/.
...
cl /nologo /EHsc /W3 /DWIN32 /D_WINDOWS ^
   /I"%SRC%\Lib_Network" ^
   /I"%SRC%\Lib_Client" ^
   /I"%SRC%\Lib_Client\G-Logic" ^
   /I"%SRC%\Lib_Client\NpcTalk" ^
   /I"%SRC%\Lib_Engine" ^
   ...
   /I"%SRC%\Tik\Include" ^
   /I"%SRC%\Tik\DXInclude" ^
   probe.cpp "%SRC%\Lib_Network\minTea.cpp" ^
   /Fe:layout-probe.exe ^
   /Foobj\ ^
...
.\layout-probe.exe > layout.json
if errorlevel 1 (
  echo [!] probe run failed
  exit /b 1
)
