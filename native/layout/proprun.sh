...
dump () { # $1 = triple, $2 = output
  "$CLANG" --target="$1" "${FLAGS[@]}" "${INC[@]}" \
      -Xclang -fdump-record-layouts -fsyntax-only "$HERE/propprobe.cpp" > "$2" 2> "$2.err" || true
  grep -E "^ *[0-9]+ \| (struct|class) |^ *\| \[sizeof=" "$2" > /dev/null || true
}

echo "== dumping aarch64 (mobile) =="
dump ${RAN_LAYOUT_TARGET:-aarch64-linux-android24} "$HERE/proplayout-arm64.txt"
echo "== dumping i686 (32-bit ABI = server word sizes) =="
dump i686-linux-android24 "$HERE/proplayout-x86.txt"
...
extract "$HERE/proplayout-arm64.txt" > "$HERE/propsizes-arm64.txt"
extract "$HERE/proplayout-x86.txt"   > "$HERE/propsizes-x86.txt"
echo "arm64 records: $(wc -l < "$HERE/propsizes-arm64.txt")   x86 records: $(wc -l < "$HERE/propsizes-x86.txt")"
join "$HERE/propsizes-x86.txt" "$HERE/propsizes-arm64.txt" | awk '$2!=$3 {print $1, "x86="$2, "arm64="$3}' > "$HERE/propmismatches.txt"
echo "MISMATCHES: $(wc -l < "$HERE/propmismatches.txt")"
head -40 "$HERE/propmismatches.txt"
